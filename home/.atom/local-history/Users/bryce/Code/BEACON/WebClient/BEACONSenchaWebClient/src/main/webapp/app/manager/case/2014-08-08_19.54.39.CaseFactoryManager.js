...
Ext.define('Beacon.manager.case.CaseFactoryManager', {
  ...
  createActivityFromCase: function(practiceModel, theCase) {
    ...
    newActivity.addCode(theCase.get('COMMUNICATION_METHOD'));
    var reason = theCase.get('REASON');
    if(reason) {
      newActivity.addCode(reason);
    }

    var scheduledStart = theCase.get('scheduledStart');
    var scheduledEnd = theCase.get('scheduledEnd');

    if(scheduledStart && scheduledEnd) {
      newActivity.set('actualStart', scheduledStart);
      newActivity.set('actualEnd', scheduledEnd);

      var minutesBetween = ((scheduledEnd - scheduledStart) / 1000) / 60;
      newActivity.set('billableMinutes', minutesBetween);
    }

    var firstTopic = newActivity.activities().getAt(0);
    firstTopic.set('description', theCase.get('description'));
    newActivity.populateCodes();
